...
function applyAssetRoute() {
    ...
    if (
        assetState.routeArea &&
        workAreaFilter
    ) {
        const matchedWorkArea =
            assetState.masterData.workAreas.find(
                area =>
                    String(area.code || '')
                        .trim()
                        .toUpperCase() ===
        ...
        if (matchedWorkArea) {
            workAreaFilter.value =
                String(matchedWorkArea.id);
        }
    }

    window.requestAnimationFrame(() => {
        if (assetState.routeView === 'search') {
...
function bindAssetEvents() {
    ...
    document
        .getElementById('assetSearchInput')
        ?.addEventListener('input', () => {
            window.clearTimeout(assetState.searchTimer);
            const searchLength = String(
                document.getElementById('assetSearchInput')?.value || ''
            ).trim().length;
            if (searchLength === 1) return;
            assetState.searchTimer = window.setTimeout(async () => {
                assetState.page = 1;
                await loadAssets();
            }, 350);
        });
    ...
    document
        .getElementById('assetPageSize')
        ?.addEventListener('change', async (event) => {
            const size = Number(event.currentTarget.value);
            assetState.limit = [25, 50, 100].includes(size) ? size : 25;
            assetState.page = 1;
            await loadAssets();
        });

    document
        .getElementById('assetRefreshButton')
        ?.addEventListener('click', async () => {
            await Promise.all([loadAssets(), loadAssetSummary()]);
        });

    document.querySelectorAll('[data-status-filter]').forEach((card) => {
        const applyFilter = async () => {
            const select = document.getElementById('assetStatusFilter');
            if (!select) return;
            select.value = card.dataset.statusFilter || '';
            assetState.page = 1;
            updateSummaryCardSelection();
            await loadAssets();
        };
...
async function loadAssets() {
    const loadSequence = ++assetState.loadSequence;
    assetState.loading = true;
    document.getElementById('assetRefreshButton')?.setAttribute('aria-busy', 'true');
    ...
    try {
        ...
        if (loadSequence !== assetState.loadSequence) return;
        ...
        renderAssetRange(data.pagination);
        ...
        if (loadSequence === assetState.loadSequence) {
            assetState.loading = false;
            document.getElementById('assetRefreshButton')?.removeAttribute('aria-busy');
        }
    }
}
...
function renderAssetTable(items) {
    ...
    items.forEach((asset) => {
        ...
        const provisionalCode = String(asset.asset_code || '').startsWith('TMP-');
        appendPrimarySecondaryCell(
            row,
            asset.asset_code,
            [provisionalCode ? 'รหัสชั่วคราว' : '', asset.barcode]
                .filter(Boolean)
                .join(' · '),
            `asset-code-cell${provisionalCode ? ' is-provisional' : ''}`
        );
        appendPrimarySecondaryCell(
            row,
            asset.asset_name,
...
function appendPrimarySecondaryCell(row, primary, secondary, className = '') {
    const cell = document.createElement('td');
...
function renderAssetRange(pagination) {
    const total = Number(pagination.total) || 0;
    const page = Number(pagination.page) || 1;
    const limit = Number(pagination.limit) || assetState.limit;
...
function updateSummaryCardSelection() {
    ...
    document.querySelectorAll('[data-status-filter]').forEach((card) => {
        const isSelected = card.dataset.statusFilter === selectedStatus;
...
async function loadAssetSummary() {
    try {
        const response = await window.authFetch(
            `${window.APP_CONFIG.API_BASE_URL}/api/assets/summary`,
            { method: 'GET', cache: 'no-store' }
        );
        const data = await response.json();
        if (!response.ok || !data.success) {
            throw new Error(data.message || 'โหลดข้อมูลสรุปทรัพย์สินไม่สำเร็จ');
        }

        setText('totalAssetCount', formatNumber(data.summary.total));
        setText('inUseAssetCount', formatNumber(data.summary.inUse));
        setText('spareAssetCount', formatNumber(data.summary.spare));
        setText('attentionAssetCount', formatNumber(data.summary.attention));
    } catch (error) {
