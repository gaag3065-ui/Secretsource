document.addEventListener('DOMContentLoaded', () => {
    ...
    const state = { channels: [], conversations: [], activeChannel: 'all', activeConversation: null };
    ...
    el.replyForm.addEventListener('submit',async event=>{event.preventDefault();const text=el.replyInput.value.trim();if(!text||!state.activeConversation)return;el.sendButton.disabled=true;try{await request(`/api/line-oa/conversations/${encodeURIComponent(state.activeConversation.id)}/messages`,{method:'POST',body:JSON.stringify({channelId:state.activeConversation.channelId,text})});el.replyInput.value='';await openConversation(state.activeConversation.id,state.activeConversation.channelId);notify('ส่งข้อความแล้ว');}catch(error){notify(error.message,true);}finally{el.sendButton.disabled=false;}});
    load();
