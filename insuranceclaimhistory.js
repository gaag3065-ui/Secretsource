...
let outcomeModalReturnFocus = null;
let outcomeModalPreviousBodyOverflow = '';
const OPEN_CASE_WAITING_STATUS =
    'เปิดเคสและแจ้งประกันแล้ว รอเข้ารับการรักษา';

function getUniqueHistoryCaseCount(history) {
    if (!Array.isArray(history)) return 0;

    return new Set(
        history
            .map((item) => String(item?.CaseIdNew || '').trim())
            .filter(Boolean)
    ).size;
}

function updateHistoryResultCount(history) {
    const countElement =
        document.getElementById('historyResultCount');

    if (!countElement) return;

    const caseCount = getUniqueHistoryCaseCount(history);
    const historyCount = Array.isArray(history) ? history.length : 0;

    countElement.textContent =
        `พบ ${caseCount.toLocaleString('th-TH')} เคส`;
    countElement.title =
        `${caseCount.toLocaleString('th-TH')} เคส จาก ${historyCount.toLocaleString('th-TH')} รายการประวัติที่กำลังแสดง`;
    countElement.dataset.count = String(caseCount);
}
...
function buildOpenCaseEvidenceName(file) {
    const numericCaseId =
        String(currentActiveCaseId || '').replace(/[^0-9]/g, '');
    const employeeName = getSafeUpdateEmployeeName();
    const extension = getImageExtension(file);

    if (!numericCaseId) {
        throw new Error('ไม่พบเลขเคส ID สำหรับตั้งชื่อหลักฐานเปิดเคส');
    }

    return `Opencase ${numericCaseId}_${employeeName}.${extension}`;
}
//#endregion
...
async function uploadOpenCaseEvidence(file) {
    if (!file) return '';

    const fileName = buildOpenCaseEvidenceName(file);
    const fileData = await fileToDataUrl(file);
    ...
    if (!response.ok || !result.success) {
        throw new Error(
            result.message || `อัปโหลด ${fileName} ไม่สำเร็จ`
        );
    }
...
window.uploadAllUpdateEvidence = async function () {
    const openCaseMode =
        ...
        OPEN_CASE_WAITING_STATUS;

    if (openCaseMode) {
        const openCaseFile =
            document.getElementById('openCaseEvidenceFile')
                ?.files?.[0] || null;

        return {
            openCaseUrl:
                await uploadOpenCaseEvidence(openCaseFile),
            receiptUrl: '',
...
function setOutcomeEvidenceMode(selectedValue) {
    const openCaseMode = selectedValue === OPEN_CASE_WAITING_STATUS;
    const openCaseGrid =
        document.getElementById('ocOpenCaseUploadGrid');
    const standardGrid =
        document.getElementById('ocStandardUploadGrid');

    if (openCaseGrid) openCaseGrid.hidden = !openCaseMode;
    if (standardGrid) standardGrid.hidden = openCaseMode;
}
...
window.toggleOcOtherInput = function (selectedValue) {
    ...
    setOutcomeEvidenceMode(selectedValue);
};
//#endregion
...
window.injectNewRowToTableRealtime = function (payloadData) {
    ...
    updateHistoryResultCount(window.currentIndividualHistory);
...
//#region
        window.closeOutcomeUpdateModal = function () {
            const modal = document.getElementById('outcomeUpdateModal');
            if (!modal) return;

            modal.style.setProperty('display', 'none', 'important');
            document.body.style.overflow =
                outcomeModalPreviousBodyOverflow;
            ...
            document
                ...
                .forEach((button) => {
                    button.textContent = 'อัปเดตผลการรักษา';
                });

            outcomeModalReturnFocus?.focus?.();
        };

        if (!window.outcomeModalEscapeBound) {
            window.outcomeModalEscapeBound = true;
            document.addEventListener('keydown', (event) => {
                const modal =
                    document.getElementById('outcomeUpdateModal');
        ...
        toggleOcOtherInput(''); 
            ...
            if (modal) {
                ...
                window.setTimeout(() => {
                    document.getElementById('ocStatusSelect')?.focus();
                }, 0);
            }
...
//#region
        ...
        if (outcomeFormEl) {
            ...
            document.getElementById('modalOutcomeForm').addEventListener('submit', async (e) => {
                ...
                const selectedStatus =
                    document.getElementById('ocStatusSelect')?.value || '';
                const openCaseMode =
                    selectedStatus === OPEN_CASE_WAITING_STATUS;
                const requiredEvidenceInput = document.getElementById(
                    openCaseMode
                        ? 'openCaseEvidenceFile'
                        : 'closeCaseEvidenceFile'
                );

                if (!requiredEvidenceInput?.files?.length) {
                    alert(
                        openCaseMode
                            ? 'กรุณาอัปโหลดรูปหลักฐานการแจ้งเคสประกันก่อนยืนยัน'
                            : 'กรุณาอัปโหลดรูปหลักฐานการปิดเคสก่อนยืนยัน'
                    );

                    const requiredEvidenceLabel = document.querySelector(
                        `label[for="${requiredEvidenceInput?.id || ''}"]`
                    );

                    requiredEvidenceLabel?.scrollIntoView({
                        behavior: 'smooth',
                        block: 'center'
                    });

                    requiredEvidenceLabel?.classList.add(
                        'upload-required-error'
                    );

                    setTimeout(() => {
                        requiredEvidenceLabel?.classList.remove(
                            'upload-required-error'
                        );
                    }, 2500);
                ...
                const caseId =
                    document.getElementById('ocCaseId')?.value || '-';
                const confirmed = confirm(
                    `ยืนยันการอัปเดตผลการรักษา เคส ID: ${caseId}\n\n` +
                    `ขั้นตอนใหม่: ${selectedStatus}`
                );
        ...
        const payload = {
            ...
            openCaseImageUrl: evidenceUrls.openCaseUrl || '',
        ...
        DocumentsAttached: [
            evidenceUrls.openCaseUrl
                ? `OPEN_CASE|${evidenceUrls.openCaseUrl}`
                : '',
...
function renderFlatHistoryTable(history) {
    ...
    updateHistoryResultCount(history);
    const tableBody =
...
// ย่อหรือขยายเฉพาะตารางประวัติ โดยไม่กระทบกล่องอื่นบนหน้าประกัน
function initHistoryTableZoom() {
    const surface =
        document.getElementById('historyTableZoomSurface');
    const viewport =
        document.getElementById('historyTableViewport');
    const range =
        document.getElementById('historyZoomRange');
    const output =
        document.getElementById('historyZoomValue');
    const zoomOutButton =
        document.getElementById('historyZoomOutButton');
    const zoomInButton =
        document.getElementById('historyZoomInButton');
    const resetButton =
        document.getElementById('historyZoomResetButton');
    ...
    const minimum = Number(range.min) || 60;
    const maximum = Number(range.max) || 160;
    const step = Number(range.step) || 10;
    const storageKey = 'insurance-history-table-zoom';

    const normaliseZoom = value => {
        ...
        const safeValue = Number.isFinite(numericValue)
            ...
            : fallbackValue;
        const steppedValue =
            Math.round(safeValue / step) * step;
    ...
    const savePreference = value => {
        try {
            ...
            // หน้าเว็บยังใช้งานได้ตามปกติ แม้เบราว์เซอร์ปิดพื้นที่จัดเก็บ
        }
    };

    const readPreference = () => {
        try {
            return normaliseZoom(
                localStorage.getItem(storageKey) || 100
            );
        } catch {
            return 100;
        }
    ...
    const applyZoom = (value, persist = true) => {
        const nextValue = normaliseZoom(value);

        surface.style.setProperty(
            '--history-table-zoom',
            String(nextValue / 100)
        );
        range.value = String(nextValue);
        output.value = `${nextValue}%`;
        output.textContent = `${nextValue}%`;
        zoomOutButton.disabled = nextValue <= minimum;
        zoomInButton.disabled = nextValue >= maximum;
    ...
    const adjustZoom = direction => {
        applyZoom(Number(range.value) + (direction * step));
    };
    ...
    // Ctrl + ล้อเมาส์ภายในตาราง ทำงานคล้ายการซูมใน Excel
    ...
    applyZoom(readPreference(), false);
}
//#endregion
